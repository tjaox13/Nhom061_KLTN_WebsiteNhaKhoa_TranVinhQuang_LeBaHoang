...
import SidebarConfigUserVi from "./SidebarConfigUserVi";
...
export default function DashboardSidebar({ isOpenSidebar, onCloseSidebar }) {
  ...
  useEffect(() => {
    ...
    if (userInfo && userInfo.roleId && userInfo.roleId === USER_ROLE.USER) {
      setRole("USER");
    }
    if (
      userInfo &&
      ...
      userInfo.roleId !== USER_ROLE.USER
    ) {
      setRole("");
    }
  }, []);
  ...
  const renderContent = (
    ...
    >
          ...
          <AccountStyle>
            ...
            <Box sx={{ ml: 2 }}>
              ...
              <Typography variant="body2" sx={{ color: "text.secondary" }}>
                {role && language === "en" && role === "ADMIN" ? "ADMIN" : ""}
                {role && language === "en" && role === "DOCTOR" ? "DOCTOR" : ""}
                {role && language === "en" && role === "USER"
                  ? "Receptionist"
                  : ""}

                {role && language === "vi" && role === "ADMIN"
                  ? "QUẢN TRỊ VIÊN"
                  : ""}
                {role && language === "vi" && role === "DOCTOR" ? "BÁC SĨ" : ""}
                {role && language === "vi" && role === "USER" ? "LỄ TÂN" : ""}
              </Typography>
            </Box>
          </AccountStyle>
        </Link>
      ...
      {/* sidebar admin */}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.ADMIN &&
        language == "vi" && <NavSection navConfig={sidebarConfigVi} />}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.ADMIN &&
        language == "en" && <NavSection navConfig={sidebarConfig} />}

      {/* sidebar doctor */}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.DOCTOR &&
        language == "vi" && <NavSection navConfig={SidebarConfigDoctorVi} />}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.DOCTOR &&
        language == "en" && <NavSection navConfig={sidebarConfigDoctor} />}
      {/* sidebar user */}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.USER &&
        language === "vi" && <NavSection navConfig={SidebarConfigUserVi} />}
      {userInfoState &&
        userInfoState.roleId === USER_ROLE.USER &&
        language === "en" && <NavSection navConfig={SidebarConfigUser} />}
    ...
    <RootStyle>
      {!isDesktop && (
        <Drawer
          ...
          PaperProps={{
            sx: { width: DRAWER_WIDTH, zIndex: 999 },
          }}
        >
          {renderContent}
        </Drawer>
