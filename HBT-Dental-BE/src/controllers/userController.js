...
let createUserForAdmin = async (req, res) => {
  let message = await userService.createUserForAdmin(req.body);
  return res.status(200).json(message);
};

let editUserForAdmin = async (req, res) => {
  let data = req.body;
  let message = await userService.editUserForAdmin(data);
  return res.status(200).json(message);
};
